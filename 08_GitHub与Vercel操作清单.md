...
- [ ] 确认真实商家允许在公开作品集展示名称、地址、菜品和大众点评链接；否则先匿名化，并避免让真实数据留在公开Git历史。
...
3. 首次提交建议说明：`feat: publish interactive AI PM portfolio prototype`。
4. 绑定GitHub远端并推送主分支。
...
获得兼容接口密钥后，在Vercel Project Settings → Environment Variables配置：

- `AI_API_KEY`
- `AI_BASE_URL`
- `AI_MODEL`

配置后重新部署，并分别验证正常调用、超时、错误响应和本地降级。不得把密钥填写到代码、README或以 `NEXT_PUBLIC_` 开头的变量中。
